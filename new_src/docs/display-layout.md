...
| Element | Size | Notes |
|---|---|---|
| Weather icon | ~20×20 or 24×24 px | Pre-rendered 1-bit bitmaps. Set of ~6–8 icons: sun, cloud, partial cloud, rain, snow, thunderstorm, fog, night/moon. |
...
| Element | Font | Status |
|---|---|---|
| Hour number | Regular 28 px (Bold preferred — needs Bold TTF) | `fira_sans_regular_28` — exists |
...
**Current status:** all weather conditions share a single placeholder icon (a
20×20 filled circle) generated by `new_src/build/generate_icons.py`. This
confirms the bitmap blit pipeline end-to-end. Real icons should be added as
entries in `generate_icons.py`’s `ICONS` dict; the condition aliases in
`new_src/src/assets/icons/__init__.py` then point to the real modules.
